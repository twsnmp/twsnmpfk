...
func IsCAValid() bool {
	return rootCACertificate != nil && rootCAPrivateKey != nil
}
...
func CreateCertificateRequest(req *CSRReqEnt, file string) error {
	...
	return outPem(keyBytes, strings.Replace(file, ".csr", ".key", -1), pemKeyType)
}
...
func createRootCACertificate() error {
	...
	datastore.UpdateCert(&datastore.CertEnt{
		ID:          fmt.Sprintf("%x", sn),
...
func outPem(data []byte, filename, pemType string) error {
