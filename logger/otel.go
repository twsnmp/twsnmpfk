...
func oteld(stopCh chan bool) {
	...
	if c, ok := config.(*otlpreceiver.Config); ok {
		c.HTTP.ServerConfig.Endpoint = fmt.Sprintf(":%d", datastore.OTelHTTPPort)
		c.GRPC.NetAddr.Endpoint = fmt.Sprintf(":%d", datastore.OTelgRPCPort)
		if datastore.OTelCert != "" && datastore.OTelKey != "" {
			c.HTTP.ServerConfig.TLSSetting = &configtls.ServerConfig{
				Config: configtls.Config{
					CertFile: datastore.OTelCert,
					KeyFile:  datastore.OTelKey,
				},
			}
			c.HTTP.ServerConfig.TLSSetting.KeyFile = datastore.OTelKey
			...
			if datastore.OTelCA != "" {
				c.HTTP.ServerConfig.TLSSetting.ClientCAFile = datastore.OTelCA
				c.GRPC.TLSSetting.ClientCAFile = datastore.OTelCA
