...
macOSの省電力機能により、画面ロックやスクリーンセーバー中に通信が一時停止して監視タイムアウトが発生するのを防ぐため、以下の設定を行ってください。
...
- **Sigmaルール脅威検知・コンプライアンス監査**: 75の組み込みSigmaルールによるSyslog脅威検知（重大度・タグ・タイムライン・AI解説）、Sigmaポーリング監視、カスタムルール自動読込。
- **完全ローカルLLM組み込み（tensai）**: 外部API不要でオフライン動作するローカルLLMエンジンと、モデル/WebGPUネイティブライブラリ管理ダイアログ。
- **Parquetログ保存・bbolt移行**: 列指向Apache Parquetによる高圧縮・高速クエリ・日次フォルダー高速削除、既存ログ移行ダイアログ。
- **twFathom風KPIカード・描画アイテム刷新**: スパークライン付きKPIカード、リアルタイムプレビュー、推奨サイズボタン。
- **トポロジー自動探索・ライン自動接続・自動レイアウト**: LLDP/CDP/STP/FDB/ARP探索、ライン自動接続、マップ右クリックからのレイアウト最適化・Undo、SNMP複数設定対応。
- **異常検知アルゴリズム拡張と表記統一**: Autoencoder, LSTM, LOF, マハラノビス距離等を追加、Syslogレポートへの異常検知移植、「異常検知」へUI統一。
- **障害通知の依存関係判定**: 根本原因ノードと影響ノードの自動判定によるアラートストーム抑止。
- **監視性能・macOS常時監視対応**: ARP Watch平滑化、PING/TCP送受信分離、macOS省電力スロットリング回避手順。

---

### Sigmaルールによる脅威検知 (v2.4.0)

Syslogメッセージからサイバー攻撃やコンプライアンス違反を自動検知します。

- **SyslogレポートのSigma検知タブ**:
  - 重大度別件数（Critical, High, Medium, Low）
  - 上位検知ルールおよびタグランキング
  - 検知タイムライン推移グラフ
  - ルール別検知詳細テーブル
  - AI（LLM）による脅威分析と推奨対処解説
- **Sigmaポーリング監視**:
  - 受信ログストリームをリアルタイム監視
  - 定義済みテンプレート「Syslog脅威検知(Sigma)」完備
- **カスタムルールの配置**:
  - データフォルダー（`datastore/sigma`）にYAMLファイルを置くだけで自動ロード

---

![h:400 center](./images/ja/syslog_sigma.png)

---

### ローカルLLM & Parquetログ保存 (v2.4.0)

- **ローカルLLM（tensaiパッケージ）**:
  - 外部クラウドAPIや外部サーバーなしで、PCローカル環境でLLMを実行
  - 「ローカルモデル管理」ダイアログでGGUFモデルのダウンロード・選択
  - WebGPUネイティブライブラリの自動管理によるハードウェアアクセラレーション
- **Apache Parquetログ保存形式**:
  - Syslog, SNMP Trap, NetFlow, sFlow, ARP, ポーリングログをParquet形式で保存
  - 従来のbboltに比べ高圧縮・高速検索
  - 保存日数を経過した古いログを日付フォルダー単位で超高速クリーンアップ
  - 既存のbboltデータベースからParquetへの一括安全移行機能

---

![h:400 center](./images/ja/llm_local_model.png)

---

![h:400 center](./images/ja/log_migration.png)

---

### KPIカード & マップ・自動発見の強化 (v2.4.0)

- **twFathom風KPIカード描画アイテム**:
  - スパークライン折れ線、ステータス連動アクセントバー、特大メトリック数値表示
  - 描画アイテム編集時のリアルタイムプレビュー、推奨サイズボタン、サイズプリセット
- **トポロジー探索とライン自動接続**:
  - LLDP, CDP, STP, FDB, ARPを活用した高精度トポロジー探索
  - 自動発見時のライン自動接続オプション（確実な場合のみ/推測でも接続）
  - 「接続先を探す」ダイアログでの確信度・判定理由表示、AI推測
- **マップ自動レイアウトと最適化**:
  - 自動発見時のレイアウト自動適用（階層型、クラスタ型、デバイス種別型）
  - マップ右クリックメニューからのレイアウト最適化とUndo（元に戻す）
- **SNMP複数設定対応**:
  - 自動発見時に複数のSNMP認証情報を登録して自動試行

---

![h:400 center](./images/ja/map_drawitem_kpi.png)

---

![h:400 center](./images/ja/map_discover_v24.png)

---

![h:400 center](./images/ja/map_layout_menu.png)

---

![h:400 center](./images/ja/node_find_neighbor.png)

---

### v2.3.0 の新機能

- **STUNによるグローバルIP確認と監視**: アドレス管理（Address）画面に `[STUN]` ボタンを追加し、STUNサーバーを利用した外部グローバルIP・外側ポート・逆引きホスト名・ローカルアドレス・RTT・GeoIPの確認機能、およびIP変更を常時監視する新ポーリング種別 `stun`（IPv4 / IPv6、テンプレート完備、AIアシスト対応）を追加。
- **PING機能の拡張（Smokeping・MTR・AI解説）**: 通常Ping/Smoke/トレースルート/MTRの4動作モード切替、Smokepingジッター・パケットロス可視化、MTRホップ別リアルタイム測定と統計チャート、AI（LLM）によるPING/MTR診断解説機能を追加。
- **Smokepingポーリング監視**: PINGポーリングに `smoke` モードを追加（連続送信回数設定、豊富な判定変数、テンプレート、AIアシスト対応）。
- **ダウンタイム・SLA集計とAI解説**: イベントログレポート、ノードレポート、ポーリングレポートに「ダウンタイム・SLA」タブを追加（SLA稼働率 %、障害回数、MTTR、ダウンタイム集計、AI解説対応）。復帰ログにダウンタイム時間を自動記録。
- **ノード保存時のアドレス即時解決**: ノード設定保存直後にアドレスモードに応じたIP/MACアドレスの即時解決・補完を実行。

### v2.2.0 の新機能

- **全レポート画面のAI解説**: ノード、IPAM/アドレス、ARP、イベントログ、Syslog、Trap、NetFlow、sFlow、sFlow Counter、MQTT、ポーリング、ネットワークの全12種類のレポートダイアログフッター（閉じるボタン左側）に `[AI解説]` ボタンを追加。表示中のタブデータ（リソース、ストレージ、プロセス、通信フロー、ログ統計等）をLLMに送信し、障害分析・過負荷検知・推奨対処策を出力（基本情報タブを除く）。
...
|自動レイアウト|接続トポロジーや種別に基づきマップ配置を自動最適化します（階層型、クラスタ型、デバイス種別型）。|
...
マップ上の空白部分を右クリックして「自動レイアウト」を選択すると、ネットワークのトポロジー階層（ツリー）、サブネットクラスタ（ハブ＆スポーク）、またはデバイス種別（カテゴリ別）に合わせてノードの配置を瞬時に自動最適化します。「レイアウトを元に戻す」で直前の配置へ即座に復元（Undo）できます。
...
LLDP、CDP、STP、FDB、ARPテーブルなどのトポロジー情報を探索し、ネットワーク機器やノード間の接続関係候補を表示します。確信度（高・中・推測）や判定理由の確認、AIによる接続推測、および「全ライン接続」ボタンによる一括接続が可能です。
...
|ノード種別自動判定|ノードの種類（サーバー、ルーター等）を自動判定します。|
|AI判定を利用|AI（LLM）を活用してノード種別を推測判定します。|
|ライン自動接続|トポロジー探索によりノード間ラインを自動接続します（確実な場合のみ/推測でも接続）。|
|自動レイアウト|発見完了時にノード配置を自動最適化します（階層型、クラスタ型等）。|
|追加SNMP設定|複数のSNMP設定を登録して優先度順に試行します。|
|＜開始＞|自動発見を開始します。|
...
---
### 描画アイテム（ポーリング結果：KPIカード）
ポーリング結果をtwFathom風のモダンなKPIカードとして表示します。スパークライン折れ線、ステータス連動アクセントバー、特大数値表示に対応し、編集ダイアログでリアルタイムプレビューと推奨サイズ選択が可能です。
...
![h:400 center](./images/ja/map_ping_mtr.png)

##### MTR AI解説
MTRの経路情報、ホップ毎の遅延・パケットロス統計データをAI (LLM)に送信し、自動でネットワーク経路の障害箇所特定や要因分析、改善アドバイスを出力します。

![h:400 center](./images/ja/map_ping_mtr_ai.png)
...
![h:400 center](./images/ja/map_ping_smokeping.png)
...
対象ノードのポーリング障害ログを基に、全体の稼働率（SLA %）、障害数、MTTR、最大/累計ダウンタイム、およびポーリング別のダウンタイム・SLA集計グラフと詳細テーブルを表示するレポートです。フッターの `[AI解説]` ボタンでLLMによる分析・改善提案も利用可能です。

![h:400 center](./images/ja/node_list_downtime.png)
...
---
### ダウンタイム・SLA
ポーリングログを基に、該当ポーリングの稼働率（SLA %）、障害発生回数（継続中件数）、MTTR（平均復旧時間）、最大/累計ダウンタイムを集計表示します。ログモードでログを保存する設定にしている場合のみ表示されます。
<!-- _class: tinytext -->

![h:400 center](./images/ja/polling_report_downtime.png)
...
|名前|ポーリングの名前です。|
|レベル|ポーリングの障害レベルです。|
|種別|ポーリングの種類です。PING,SNMP,TCP,STUNなど|
...
STUNサーバーを利用して、外部グローバルIPアドレス、外側ポート番号、逆引きホスト名、送信元アドレス、応答時間（RTT）、位置情報（GeoIP）を取得・表示します。

![h:380 center](./images/ja/address_stun.png)
...
![h:400 center](./images/ja/eventlog_downtime_sla.png)
...
SyslogメッセージからSigmaルールに基づいてサイバー攻撃や不正アクセス、コンプライアンス違反を検知・分析したレポートです。
...
機械学習アルゴリズム（Mahalanobis距離、Isolation Forest等）と特徴抽出手法（TF-IDF等）により、Syslogメッセージの異常スコアを分析したレポートです。

![h:400 center](./images/ja/syslog_anomaly.png)
...
|ログ保存形式|ログデータの保存形式（Parquet / bbolt）を表示します。既存のbbolt環境では「Parquetへ移行」ボタンで一括データ移行が可能です。|
---
|項目|内容|
|----|----|
...
---
### ログ保存先の移行 (bbolt → Parquet)

従来のbboltデータベースに保存されている各種ログ（Syslog、TRAP、ポーリングログ、NetFlow、sFlow等）を高速・高圧縮なApache Parquet形式へ安全に一括移行する機能です。
...
---
### ローカルLLMモデル & GPU管理 (tensai)

Ollama等の外部サーバーを別途用意することなく、アプリ単体で完全ローカル動作するLLMエンジン（tensai）を統合しています。WebGPUによるGPU高速推論およびGGUFモデルのダウンロード・管理に対応しています。
