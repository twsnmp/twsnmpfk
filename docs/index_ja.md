...
### STUNによるグローバルIP確認と監視 (v2.3.0)
- **アドレス管理でのSTUN情報表示**: アドレス管理（Address）画面に `[STUN]` ボタンを追加しました。STUNサーバーを利用して、ローカルネットワークからインターネットへアクセスする際の外部グローバルIPアドレス、外側ポート番号、DNS逆引きホスト名、ローカル送信元アドレス、応答時間（RTT）、位置情報（GeoIP）を取得・表示し、コピーやGoogle Maps/VirusTotal連携が可能です。IPv4およびIPv6に対応しています。
