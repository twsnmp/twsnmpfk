...
- **Sigma Rule Engine Integration**: Natively integrated the open-standard Sigma rule engine for log threat detection. Includes 75 embedded rule packs covering Linux authentication/system security, network threats, Windows AD/client attacks, web application attacks, and Wazuh compliance auditing.
...
![Syslog Sigma Detection](images/en/syslog_sigma.png)
...
![Local Model Manager](images/en/llm_local_model.png)
...
![Log Migration Dialog](images/en/log_migration.png)
...
![KPI Card Drawing Item](images/en/map_drawitem_kpi.png)
...
- **Automated Layouts & Right-Click Layout Optimization**: Added layout algorithm selection during auto discovery (Hierarchical / Tree, Cluster / Hub & Spoke, Categorized by device type) and right-click canvas menu options for layout optimization with full Undo support.
...
![Discovery v2.4.0](images/en/map_discover_v24.png)
![Map Layout Menu](images/en/map_layout_menu.png)
![Find Neighbors](images/en/node_find_neighbor.png)
...
![Syslog Anomaly Detection](images/en/syslog_anomaly.png)
...
![](./images/en/map_ping_smokeping.png)
...
An aggregate report analyzing overall node SLA (Availability %), total incidents count (with ongoing status), MTTR (Mean Time To Recovery), and max/total downtime based on the node's polling failure and recovery logs.
Displays a dual-axis ranking chart of downtime (min) and SLA (%) per polling item on the left, alongside a detailed breakdown table on the right. Clicking `[AI Explain]` in the footer generates AI-powered availability analysis and operational recommendations.

![](./images/en/node_list_downtime.png)
...
Calculates and displays overall SLA (Availability %), total incidents count (with ongoing status), MTTR (Mean Time To Recovery), and max/total downtime based on polling logs.
Displays an incident timeline bar chart on the left, alongside a detailed breakdown table on the right. Only displayed when log mode is enabled.

![](./images/en/polling_report_downtime.png)
...
![](./images/en/eventlog_downtime_sla.png)
