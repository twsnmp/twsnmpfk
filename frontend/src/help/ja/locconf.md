...
* **スタイル**
  地図を表示するためのスタイル（MapLibre GL JSのスタイル定義URLまたはJSONオブジェクト）。右上の「スタイルプリセット」プルダウンから、おすすめの地図スタイルを選択して入力できます。
* **中央座標**
  地図を開いた際の基準となる中心点の座標（「経度,緯度」の順でカンマ区切りで指定。例: `135.338,39.614`）。
* **ズーム**
  地図の初期表示ズームレベル（拡大率）。
* **アイコンサイズ**
  地図上に表示するノードアイコンのピクセルサイズ（16px, 24px, 32px, 48px, 64px から選択可能）。
