<script lang="ts">
  ...
  const watch = async () => {
    ...
    const src = d[0].SrcAddr || d[0].Remote || "";
    const dst = d[0].DstAddr || "";
    if (hasAI) {
      aiNodeID = src;
      if (counter) {
        aiPrompt = get(_)("AIPollingAssist.PromptSFlowCounter", {
          values: {
            src: src,
            type: d[0].Type || "",
            data: d[0].Data || "",
        ...
        aiPrompt = get(_)("AIPollingAssist.PromptSFlowFlow", {
          values: {
            src: `${src}:${d[0].SrcPort || ""} (${d[0].SrcLoc || ""})`,
            dst: `${dst}:${d[0].DstPort || ""} (${d[0].DstLoc || ""})`,
            protocol: d[0].Protocol || "",
            bytes: d[0].Bytes || 0,
            packets: d[0].Packets || 0,
          },
        });
      }
      showAIAssist = true;
    ...
    polling = await GetDefaultPolling(src);
    polling.Name = counter ? `sflow counter ${src}` : `sflow ${src}->${dst}`;
    polling.Type = "netflow";
    polling.Mode = "traffic";
    polling.Filter = counter ? `Remote = '${src}'` : `SrcAddr = '${src}' and DstAddr = '${dst}'`;
    polling.Params = "";
    showPolling = true;
  };
...
<div class="flex flex-col max-h-[calc(100vh-50px)] overflow-y-auto pb-2">
  <div id="chart"></div>
  <div id="tableBase" class="mx-2 mt-2">
  ...
  <div class="flex justify-end space-x-2 mr-2 py-2">
    ...
    {#if logs.length > 0}
      ...
      {#if selectedCount > 0}
        ...
        <Dropdown bind:isOpen={addrInfoOpen} class="w-44 bg-white dark:bg-gray-700 text-gray-900 dark:text-white list-none">
          {#each addrList as a}
