<script lang="ts">
  ...
  import { GetNotifyConf,UpdateNotifyConf,TestNotifyConf} from '../../wailsjs/go/main/App';
  ...
  <form class="flex flex-col space-y-4" action="#">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>メールサーバー</span>
        <Input bind:value={conf.MailServer} placeholder="host|ip:port" required  size="sm"/>
      </Label>
      <Checkbox bind:checked={conf.InsecureSkipVerify}>サーバー証明書をチェックしない</Checkbox>
    </div>
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>ユーザー</span>
      ...
      <Label class="space-y-2">
        <span>パスワード</span>
        <Input type="password" bind:value={conf.Password}  placeholder="•••••"   size="sm" />
      </Label>
    </div>
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>送信元</span>
        <Input bind:value={conf.MailFrom} placeholder="送信元メールアドレス"  size="sm" />
      </Label>
      <Label class="space-y-2">
        <span>宛先</span>
        <Input bind:value={conf.MailTo}  placeholder="宛先メールアドレス"   size="sm" />
      </Label>
    </div>
    <Label class="space-y-2">
      <span>
    ...
    <div class="grid gap-4 md:grid-cols-4">
      <Label class="space-y-2">
        ...
        <Select items={notifyLevelList} bind:value={conf.Level} placeholder="通知レベルを選択" size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          通知間隔(秒)
        </span>
        <Input type="number" min={60} max={3600*24} step={10} bind:value={conf.Interval}  size="sm" />
      </Label>
