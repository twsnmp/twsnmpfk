...
<Modal bind:open={show} size="lg" autoclose={false} class="w-full" on:on:close={close}>
  <form class="flex flex-col space-y-4" action="#">
    ...
    <Label class="space-y-2">
      <span>マップ名</span>
      <Input bind:value={conf.MapName} placeholder="マップ名" required  size="sm"/>
    </Label>
    <div class="grid gap-4 mb-4 md:grid-cols-4">
      <Label class="space-y-2">
        ...
        <Input type="number" min={5} max={3600*24} step={1} bind:value={conf.PollInt}  size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          タイムアウト(秒)
      ...
      <Label class="space-y-2">
        ...
        <Input type="number" min={1} max={365*5} step={1} bind:value={conf.LogDays}  size="sm"/>
      </Label>
    </div>
    <div class="grid gap-4  md:grid-cols-3">
      <Label class="space-y-2">
        ...
        <Select items={snmpModeList} bind:value={conf.SnmpMode} placeholder="SNMPのモードを選択" size="sm"/>
      </Label>
      ...
      {:else}
        ...
        <Label class="space-y-2">
          <span>SNMPパスワード</span>
          <Input type="password" bind:value={conf.SnmpPassword}  placeholder="•••••"   size="sm" />
        </Label>
      {/if}
    </div>
