<script lang="ts">
  ...
  import { ExportMarkdown } from "../../wailsjs/go/main/App";
  ...
  export let title = "";
  export let analyzeFunc: (() => Promise<{ Results?: string; Error?: string }>) | undefined = undefined;
  export let exportFilename = "report_ai_explanation";
  ...
  const exportReport = async () => {
    if (!content) return;
    try {
      await ExportMarkdown(exportFilename, content);
    } catch (e: any) {
      error = e?.message || String(e);
    }
  };
  ...
  <div class="flex flex-col max-h-[70vh]">
    <div class="flex items-center space-x-2 border-b pb-2 mb-3 dark:border-gray-700">
      <span class="text-purple-600 dark:text-purple-400">
        <Icon path={icons.mdiBrain} size={1.2} />
      </span>
    ...
    <div class="flex justify-end space-x-2 mt-4">
      {#if content && !loading}
        ...
        >
          ...
          {copied ? $_("ReportAI.Copied") : $_("ReportAI.Copy")}
        </GradientButton>
        <GradientButton
          shadow
          type="button"
