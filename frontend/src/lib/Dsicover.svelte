<script lang="ts">
  ...
  import { GetDiscoverConf, GetDiscoverStats, StartDiscover,StopDiscover } from '../../wailsjs/go/main/App';
  ...
  <form class="flex flex-col space-y-4" action="#">
    ...
    <div class="grid gap-4 mb-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>開始IP</span>
        <Input bind:value={conf.StartIP} placeholder="開始IP" required  size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>終了IP</span>
        <Input bind:value={conf.EndIP} placeholder="終了IP" required  size="sm"/>
      </Label>
    </div>
    <div class="grid gap-4 mb-4 md:grid-cols-3">
      <Label class="space-y-2">
        ...
        <Input type="number" min={1} max={120} step={1} bind:value={conf.Timeout}  size="sm" />
      </Label>
      <Label class="space-y-2">
        <span>
          リトライ(回)
        </span>
        <Input type="number" min={0} max={100} step={1} bind:value={conf.Retry}  size="sm"/>
      </Label>
...
<Modal bind:open={showStats} size="lg" autoclose={false} class="w-full">
  <h3 class="mb-1 font-medium text-gray-900 dark:text-white">自動発見の進行状況</h3>
  <div class="flex flex-col space-y-4">
    <Progressbar 
      progress={(stats.Total ?  (100*stats.Sent/ stats.Total).toFixed(2) : 0) +''}
      color="blue" size="h-5" 
      labelOutside="Total:{stats.Sent + '/' + stats.Total}" 
    />
    <Progressbar 
      progress={(stats.Total ?  (100*stats.Found/ stats.Total).toFixed(2) : 0) +''}
      color="indigo" size="h-5" 
