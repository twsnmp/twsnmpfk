<script lang="ts">
  ...
  import { GetNode,GetLine,UpdateLine, DeleteLine, GetPollings} from '../../wailsjs/go/main/App';
  ...
  <form class="flex flex-col space-y-4" action="#">
    ...
    <div class="grid gap-4 mb-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>ノード1</span>
        <Input bind:value={node1.Name} placeholder="ノード1名" readonly  size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>ノード2</span>
        <Input bind:value={node2.Name} placeholder="ノード2名" readonly  size="sm"/>
      </Label>
    </div>
    <div class="grid gap-4 mb-4 md:grid-cols-2">
      <Label class="space-y-2">
        ...
        <Select items={pollingList1} bind:value={line.PollingID1} placeholder="ノード１のポーリング" size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          ポーリング2
        </span>
        <Select items={pollingList2} bind:value={line.PollingID2} placeholder="ノード2のポーリング" size="sm"/>
      </Label>
    </div>
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        ...
        <Select items={pollingList} bind:value={line.PollingID} placeholder="情報のポーリング" size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>情報</span>
    ...
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        <span>ラインの太さ</span>
        <Input bind:value={line.Width} placeholder="ラインの太さ" type="number" min="1" max="5" size="sm" />
      </Label>
      <Label class="space-y-2">
        <span>ポート</span>
