  ...
  <form class="flex flex-col space-y-4" action="#">
    ...
    <div class="grid gap-4  md:grid-cols-3">
      <Label class="space-y-2">
        ...
        <Select items={aiLevelList} bind:value={conf.HighThreshold} placeholder="レベルを選択" size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          軽度と判定するレベル
        </span>
        <Select items={aiLevelList} bind:value={conf.LowThreshold} placeholder="レベルを選択" size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          注意と判定するレベル
        </span>
        <Select items={aiLevelList} bind:value={conf.WarnThreshold} placeholder="レベルを選択" size="sm"/>
      </Label>
    </div>
