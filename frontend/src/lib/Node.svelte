  ...
  <form class="flex flex-col space-y-4" action="#">
    ...
    <div class="grid gap-4 mb-4 md:grid-cols-3">
      <Label class="space-y-2">
        <span>名前</span>
        <Input bind:value={node.Name} placeholder="ノード名" required  size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>IPアドレス</span>
        <Input bind:value={node.IP} placeholder="IPアドレス" required  size="sm"/>
      </Label>
      <Label class="space-y-2">
        <span>
          アドレスモード
        </span>
        <Select items={addrModeList} bind:value={node.AddrMode} placeholder="アドレスモードを選択" size="sm"/>
      </Label>
    </div>
    <div class="grid gap-4 mb-4 md:grid-cols-3">
      <Label class="space-y-2">
        ...
        <Select items={iconList} bind:value={node.Icon} placeholder="アイコンを選択" size="sm"/>
      </Label>
      <div class="mt-5 ml-5">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      <Label class="space-y-2">
        ...
        <Select items={snmpModeList} bind:value={node.SnmpMode} placeholder="SNMPのモードを選択" size="sm"/>
      </Label>
      <Label class="space-y-2">
    ...
    <div class="grid gap-4 md:grid-cols-2">
      ...
      <Label class="space-y-2">
        <span>SNMPパスワード</span>
        <Input type="password" bind:value={node.Password}  placeholder="•••••"   size="sm" />
      </Label>
    </div>
    <Label class="space-y-2">
      <span>公開鍵</span>
