<script lang="ts">
  ...
  import { LLMExplainCertMonitor, ExportMarkdown } from "../../wailsjs/go/main/App";
  ...
  const analyze = async () => {
    ...
    copied = false;
    try {
      const resp = await LLMExplainCertMonitor();
      if (resp.Error) {
        error = resp.Error;
      } else {
  ...
  const exportReport = async () => {
    if (!content) return;
    try {
      const filename = "certificate_ai_explanation";
    ...
    <div class="flex justify-end space-x-2 mt-4">
      {#if content && !loading}
        ...
        >
          ...
          {copied ? $_("CertAIDialog.Copied") : $_("CertAIDialog.Copy")}
        </GradientButton>
        <GradientButton
          shadow
          type="button"
