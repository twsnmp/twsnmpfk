<script lang="ts">
  ...
  import { getStateColor, getStateIcon, getStateName, levelList, timeFormat } from './common';
  let logs = [];
    ...
    {#each logs as l}
      <TableBodyRow>
        <TableBodyCell tdClass="text-xs p-1">
          <div class="flex">
            <Icon path={getStateIcon(l.Level)} color={getStateColor(l.Level)} size={1}></Icon>
            <span class="text-xs mt-1 ml-1">
              {getStateName(l.Level)}
            </span>
          </div>
        </TableBodyCell>
        <TableBodyCell tdClass="text-xs p-1">{timeFormat(new Date(l.Time /(1000*1000)),"")}</TableBodyCell>
