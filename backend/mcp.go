...
func startMCPServer() *echo.Echo {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "get_node_list", Description: "get node list from TWSNMP"}, getNodeList)
	mcp.AddTool(s, &mcp.Tool{Name: "get_network_list", Description: "get network list from TWSNMP"}, getNetworkList)
	mcp.AddTool(s, &mcp.Tool{Name: "get_polling_list", Description: "get polling list from TWSNMP"}, getPollingList)
	mcp.AddTool(s, &mcp.Tool{Name: "get_polling_log", Description: "get polling log from TWSNMP"}, getPollingLog)
	mcp.AddTool(s, &mcp.Tool{Name: "do_ping", Description: "do ping"}, doPing)
	mcp.AddTool(s, &mcp.Tool{Name: "get_MIB_tree", Description: "get MIB tree from TWSNMP"}, getMIBTree)
	mcp.AddTool(s, &mcp.Tool{Name: "snmpwalk", Description: "SNMP walk tool"}, snmpWalk)
	mcp.AddTool(s, &mcp.Tool{Name: "add_node", Description: "add node to TWSNMP"}, addNode)
	mcp.AddTool(s, &mcp.Tool{Name: "update_node", Description: "update node name,ip, positon,description or icon"}, updateNode)
	mcp.AddTool(s, &mcp.Tool{Name: "get_ip_address_list", Description: "get IP address list from TWSNMP"}, getIPAddressList)
	mcp.AddTool(s, &mcp.Tool{Name: "get_resource_monitor_list", Description: "get resource monitor list from TWSNMP"}, getResourceMonitorList)
	mcp.AddTool(s, &mcp.Tool{Name: "search_event_log", Description: "search event log from TWSNMP"}, searchEventLog)
	mcp.AddTool(s, &mcp.Tool{Name: "search_syslog", Description: "search syslog from TWSNMP"}, searchSyslog)
...
type getPollingListParams struct {
	TypeFilter     string `json:"type_filter" jsonschema:"ype_filter uses a regular expression to specify search criteria for polling type names.If blank, all pollings are searched.Type names can be ping,tcp,http,dns,twsnmp,syslog"`
...
type doPingParams struct {
	Target  string `json:"taget" jsonschema:"ping target ip address or host name"`
	Size    int    `json:"size" jsonschema:"ping packate size"`
...
func doPing(ctx context.Context, req *mcp.CallToolRequest, args doPingParams) (*mcp.CallToolResult, any, error) {
	target := getTragetIP(args.Target)
	if target == "" {
...
func snmpWalk(ctx context.Context, req *mcp.CallToolRequest, args snmpWalkParams) (*mcp.CallToolResult, any, error) {
	...
	if n := datastore.FindNodeFromName(target); n != nil {
		...
		if target == "" {
			return nil, nil, fmt.Errorf("traget not found")
		}
	}
	agent := &gosnmp.GoSNMP{
		Target:    target,
...
type addNodeParams struct {
	...
	X           int    `json:"x" jsonschema:"x positon of node"`
	Y           int    `json:"y" jsonschema:"y positon of node"`
}
...
type searchEventLogParams struct {
	...
	EndTime     string `json:"end_time" jsonschema:"nd date and time of logs to search.empty or now is current time."`
